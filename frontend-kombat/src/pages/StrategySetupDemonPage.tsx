...
export default function StrategySetupDemonPage({
  ...
  const defLightColor = shadeHex(baseColor, 60)
  const defDarkColor = shadeHex(baseColor, -18)
  const isAssassinDemon = minion.type === "ASSASSIN"
...
<div className="mt-1 flex justify-center">
  <div className="relative flex items-center justify-center group">
    ...
    "
    style={{
      borderColor: isAssassinDemon ? "rgba(168,85,247,0.4)" : hexToRgba(baseColor, 0.48),
      boxShadow: isAssassinDemon
        ? "0 0 50px rgba(168,85,247,0.8)"
        : `0 0 50px ${hexToRgba(baseColor, 0.78)}`,
    }}
    />
    ...
    "
    style={{ borderColor: isAssassinDemon ? "rgba(192,132,252,0.4)" : hexToRgba(defLightColor, 0.42) }}
    />

    <div className="absolute w-[160px] h-[160px] pointer-events-none">
      <div className="absolute top-3 left-1/2 w-2 h-2 rounded-full animate-ping opacity-80" style={{ backgroundColor: isAssassinDemon ? "#a855f7" : baseColor }} />
      <div className="absolute bottom-6 left-6 w-1.5 h-1.5 rounded-full animate-pulse opacity-70" style={{ backgroundColor: isAssassinDemon ? "#c084fc" : defLightColor }} />
      <div className="absolute top-10 right-4 w-1.5 h-1.5 rounded-full animate-ping opacity-60" style={{ backgroundColor: isAssassinDemon ? "#9333ea" : defDarkColor }} />
      <div className="absolute bottom-8 right-8 w-2 h-2 rounded-full animate-pulse opacity-70" style={{ backgroundColor: isAssassinDemon ? "#d8b4fe" : shadeHex(defLightColor, 25) }} />
    </div>
    ...
    <div
      className="
        ...
        group-hover:shadow-[0_0_65px_rgba(255,255,255,0.35)]
        z-10
      "
      style={{
        backgroundImage: isAssassinDemon
          ? "linear-gradient(90deg, #1a002b, #4c1d95, #1a002b)"
          : `linear-gradient(90deg, ${defDarkColor}, ${baseColor}, ${defDarkColor})`,
        borderColor: isAssassinDemon ? "#c084fc" : hexToRgba(defLightColor, 0.74),
        color: isAssassinDemon ? "#e9d5ff" : "#f8ead0",
        boxShadow: isAssassinDemon
          ? "0 0 35px rgba(168,85,247,0.8)"
          : `0 0 35px ${hexToRgba(baseColor, 0.8)}`,
      }}
    >
      <svg
        viewBox="0 0 24 24"
        fill="currentColor"
        className="w-6 h-6"
        style={{ color: isAssassinDemon ? "#c084fc" : defLightColor }}
      >
      ...
      <button
        ...
        style={{
          backgroundImage: isAssassinDemon
            ...
            : `linear-gradient(180deg, ${defLightColor}, ${baseColor})`,
          color: isAssassinDemon ? "#1a002b" : defDarkColor,
        }}
      >
        -
      </button>

      <span className="text-2xl font-bold tracking-widest" style={{ color: isAssassinDemon ? "#f3e8ff" : "#fff1dc" }}>
        {defenFactor}
      </span>
      ...
      <span className="text-sm tracking-[0.3em]" style={{ color: isAssassinDemon ? "#e9d5ff" : "#fff1dc" }}>
        DEF
      </span>
    </div>
